...
if (savePosition)
{    
    global.saveRoomC = room_get_name(room);
    global.savePlayerCX = objCreatorPlayer.x;    
    global.savePlayerCY = objCreatorPlayer.y;
    ...
    with (objCreatorPlayer)
    {
        if (!place_free(floor(global.savePlayerCX),global.savePlayerCY))
        {
            global.savePlayerCX += 1;
        }
        
        if (!place_free(global.savePlayerCX,floor(global.savePlayerCY)))
        {
            global.savePlayerCY += 1;
        }
        
        if (!place_free(floor(global.savePlayerCX),floor(global.savePlayerCY)))
        {
            global.savePlayerCX += 1;
            global.savePlayerCY += 1;
        }
    ...
    global.savePlayerCX = floor(global.savePlayerCX);
    global.savePlayerCY = floor(global.savePlayerCY);
...
ds_map_add(saveMap,"saveRoom",global.saveRoomC);
ds_map_add(saveMap,"savePlayerX",global.savePlayerCX);
ds_map_add(saveMap,"savePlayerY",global.savePlayerCY);
